...
async function main() {
  ...
  const generate_pin_map = await client.generateChart("generate_pin_map", {
    title: "西安著名景点",
    data: ["西安钟楼", "西安大雁塔", "秦始皇兵马俑", "华清宫"],
  });
  console.log("generate_pin_map生成完毕：", generate_pin_map);

  const generate_path_map = await client.generateChart("generate_path_map", {
    title: "西安旅游路线图",
    data: [
      { data: ["西安钟楼", "西安大唐不夜城", "西安大雁塔"] },
      { data: ["西安曲江池公园", "西安回民街"] },
    ],
  });
  console.log("generate_path_map生成完毕：", generate_path_map);
